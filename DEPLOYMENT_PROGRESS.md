...
> **專案狀態**：部署完成，全部 11 台 Automation Action 已設定
...
✅ Phase 4: 測試與驗證 (100%)
✅ Phase 5: Fortigate Automation Action 設定 (100%)
```
...
- `ac7d6dc` - fix: Watcher 容器時區設為 Asia/Taipei + README/進度更新
- `0390616` - fix: rconfig 格式檔名固定為 show_1930.txt
...
- [x] UFW 防火牆規則（11 台 Fortigate IP + 印度 2 台 + Docker 網段）
...
- 容器時區預設 UTC → 加上 `TZ: Asia/Taipei`
- show_{HHmm}.txt 動態時間與排程衝突 → 固定為 show_1930.txt
...
- [x] rconfig 格式檔案正確：`show_1930.txt`（固定檔名，對齊備份排程）
...
### Phase 5: Fortigate Automation Action 設定 (11/11 台)

#### 台灣彰化 HQ2 (172.16.11.x)

| 設備 | IP | 狀態 |
|------|-----|------|
| TWCH-HQ2-101F | 172.16.11.2 | ✅ 已完成 |
| TWCH-HQ2-201F-01 | 172.16.11.3 | ✅ 已完成 |
| TWCH-PCN-301E | 172.16.11.4 | ✅ 已完成 |
| TWCH-HQ2-60E-IOT | 172.16.11.5 | ✅ 已完成 |

#### 台灣台中 (172.23.x.x)

| 設備 | IP | 狀態 |
|------|-----|------|
| TWTC-PA-101F | 172.23.127.9 | ✅ 已完成 |
| TW-TC-FortiGate-121G | 172.23.174.9 | ✅ 已完成 |
| TW-TC-UAIC-FW | 172.23.199.251 | ✅ 已完成 |

#### 台灣台北 (172.23.x.x)

| 設備 | IP | 狀態 |
|------|-----|------|
| TW-TP-BaoYu-FortiGate60E | 172.23.94.9 | ✅ 已完成 |
| TW-TP-XinYi-FortiGate60E | 172.23.110.30 | ✅ 已完成 |

#### 印度 (172.25.x.x)

| 設備 | IP | 狀態 |
|------|-----|------|
| IN-Kavin-60E | 172.25.128.254 | ✅ 已完成 |
| IN-VSP-60E | 172.25.136.254 | ✅ 已完成 |
...
| v1.2.0 | 2026-03-25 | TZ 修正、固定檔名 show_1930.txt、全部 11 台 Automation Action 設定完成 |

---

**最後更新**: 2026-03-25
**當前狀態**: 全部完成，11/11 台 Automation Action 已設定，等待今晚 19:30 首次全量備份驗證
